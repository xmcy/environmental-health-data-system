...
export class Map extends GTemplate {
    ...
    active: any = null;
    colors: any = {
        disease01: "rgba(0, 223, 255, 0.9)",
        disease02: "rgba(255, 190, 70, 0.9)",
        disease03: "rgba(107, 217, 141, 0.9)",
        disease04: "rgba(90, 133, 231, 0.9)",
        disease05: "rgba(2, 145, 255, 0.9)"
    };
    geoCoordMap: any = {
    ...
    async getDisease() {
        let data: any = await this.service.get("/free/general/diseases/list");
        if (!data || !data.result) {
            this.disease = [];
            return;
        }
        this.disease = data.result;
        this.active = this.disease[0].code;
        this.getData();
    }
    ...
    async getData() {
        let data: any = await this.service.get(`/free/general/diseases/${this.active}`);
        if (!data || !data.result || !data.result?.countries) {
            this.chartOption.series[0].data = [];
            return;
        }
        this.chartOption.series[0].data = this.initEchart(data.result?.countries);
        this.chartOption.series[0].itemStyle.color = this.colors[this.active];
    ...
    initEchart(data: any) {
        ...
        for (let i = 0; i < arr.length; i++) {
            let geoCoord = this.geoCoordMap[arr[i].name];
            if (geoCoord) {
                res.push({
                    name: arr[i].name,
